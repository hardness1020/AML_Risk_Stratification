...
class KMSurvivalCurve:
    ...
    def return_survival_curve_bytes(self, plot=False):
        """
        Returns the survival curve as bytes.

        Args:
            plot (bool, optional): Whether to display the plot. Defaults to False.

        Returns:
            bytes: The survival curve as bytes.
        """
        ...
        for idx, (label1, label2, pvalue) in enumerate(pvalues):
            plt.annotate(f'p-value ({label1}, {label2}): {pvalue:.1E}', 
                         xy=(0.20, 0.90 - idx * 0.05), xycoords='axes fraction',
                         fontsize=10)
